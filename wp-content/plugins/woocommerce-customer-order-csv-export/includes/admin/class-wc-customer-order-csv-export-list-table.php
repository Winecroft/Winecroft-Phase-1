	...
	public function column_default( $export, $column_name ) {
		...
		switch ( $column_name ) {
			...
			case 'export_status':
				...
				$status = 'processing' === $export->status && wc_customer_order_csv_export()->is_batch_processing_enabled() ? 'paused' : $export->status;
	...
	public function column_file_actions( $export ) {
		...
		?><p>
			<?php
				...
				} elseif ( 'processing' === $export->status && wc_customer_order_csv_export()->is_batch_processing_enabled() ) {
				...
				foreach ( $actions as $action ) {
					printf( '<a class="button tips %1$s" href="%2$s" data-tip="%3$s" data-export-id="%4$s">%5$s</a>', esc_attr( $action['action'] ), esc_url( $action['url'] ), esc_attr( $action['name'] ), esc_attr( $export->id ), esc_attr( $action['name'] ) );
				}
			?>
		</p><?php
