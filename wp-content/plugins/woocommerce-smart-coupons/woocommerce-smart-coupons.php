	...
	if ( ! class_exists( 'WC_Smart_Coupons' ) ) {
		...
		class WC_Smart_Coupons {
			...
			private function __construct() {
	            ...
	            add_action( 'woocommerce_order_after_calculate_totals', array( $this, 'admin_apply_store_credit' ), 10, 2 );
			...
			public function smart_coupons_discount_total_filters() {
				if ( WC_SC_WCS_Compatibility::is_cart_contains_subscription() && WC_SC_WCS_Compatibility::is_wcs_gte( '2.0.0' ) ) {
					...
					if ( ! empty( $current_screen ) && $current_screen != 'edit-shop_order' ) {
		     ...
		     *
		 	 ...
		 	 * @param WC_Order $order
		 	 */
			public function admin_apply_store_credit( $and_taxes, $order ) {
				$order_actions = array( 'woocommerce_add_coupon_discount', 'woocommerce_calc_line_taxes', 'woocommerce_save_order_items' );

				if ( ! empty( $_POST['action'] ) && ( in_array( $_POST['action'] , $order_actions )  || ( ! empty( $_POST['post_type'] ) && $_POST['post_type'] == 'shop_order' && $_POST['action'] == 'editpost' ) ) ) {
					...
					if ( ! empty( $coupons ) ) {
						foreach ( $coupons as $item_id => $item ) {
							...
							if ( $discount_type === 'smart_coupon' ) {
								...
								} else if ( ! empty( $discount_amount = wc_get_order_item_meta( $item_id, 'discount_amount', true ) ) ) {
									$discount = $discount_amount;
								} else {
									$discount = $this->sc_get_discount_amount( $total, $coupon );
			 ...
			 * @param  float      $total
			 * @param  WC_Coupon  $coupon
			 * @return float 	  $discont
			 */
			public function sc_get_discount_amount( $total = 0, $coupon = '' ) {
				$discount = 0;
			 ...
			 * @param bool      $cart_contains_subscription
			 * @param float     $discount
				...
				?>
					...
					<div class="alignright" style="margin-top: 1px;" >
						...
						if ( ! empty( $_SERVER['QUERY_STRING'] ) ) {
							echo '<input type="hidden" name="sc_export_query_args" value="' . esc_attr( $_SERVER['QUERY_STRING'] ) . '">';
						}
						?>
			...
			public function woocommerce_export_coupons() {
				...
				if ( is_admin() && isset( $_GET['export_coupons'] ) && current_user_can( 'manage_woocommerce' ) ) {
