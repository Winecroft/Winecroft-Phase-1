...
<div id="payment_methods" class="account_form clearfix">

	<h3 class="account_form_heading">Payment Methods  <span class="togglefieldset pull-right"><i class="fa fa-angle-down"></i></span></h3>
<div>
<?php if ( $has_methods ) : ?>
						...
						<td class="woocommerce-PaymentMethod woocommerce-PaymentMethod--<?php echo esc_attr( $column_id ); ?> payment-method-<?php echo esc_attr( $column_id ); ?>" data-title="<?php echo esc_attr( $column_name ); ?>">
							...
							if ( has_action( 'woocommerce_account_payment_methods_column_' . $column_id ) ) {
								...
								foreach ( $method['actions'] as $key => $action ) {
									echo '<a href="' . esc_url( $action['url'] ) . '" class="primary_btn minibttn ' . sanitize_html_class( $key ) . '">' . esc_html( $action['name'] ) . '</a>&nbsp;';
								}
							}
							?>
						</td>
...
<?php if ( WC()->payment_gateways->get_available_payment_gateways() ) : ?>
	<a class="primary_btn minibttn" href="<?php echo esc_url( wc_get_endpoint_url( 'add-payment-method' ) ); ?>"><?php esc_html_e( 'Add payment method', 'woocommerce' ); ?></a>
<?php endif; ?>
</div>
